-- supabase/migrations/20260717_advanced_circuit_breaker.sql

ALTER TABLE faenas DROP CONSTRAINT IF EXISTS faenas_estado_check;
ALTER TABLE faenas ADD CONSTRAINT faenas_estado_check CHECK ((estado = ANY (ARRAY['programada'::text, 'en_curso'::text, 'finalizada'::text, 'cancelada'::text, 'cancelada_cliente'::text, 'cancelada_gebo'::text, 'incidente'::text, 'asignada'::text, 'ofrecida'::text, 'chofer_en_camino'::text, 'chofer_llegó'::text, 'siniestrado'::text, 'pendiente_auditoria_admin'::text])));


CREATE OR REPLACE FUNCTION public.finalizar_faena_sync(p_faena_id uuid, p_odometro_fin integer, p_gps_km numeric, p_foto_url text)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
DECLARE
    ...
    v_max_tarifa_seguridad CONSTANT DECIMAL := 15000.00;
    v_estado_final VARCHAR := 'finalizada';
BEGIN
    -- Obtener faena
    ...
    -- *** CIRCUIT BREAKER CONTABLE ***
    IF v_costo_final > v_max_tarifa_seguridad THEN
        v_estado_final := 'pendiente_auditoria_admin';
    END IF;
    ...
    -- Impactar saldos solo si no es auditoría
